...
export default function PredictionCard({ prediction, onOpenWhatIf }) {
  ...
  return (
    <div className={`rounded-2xl border ${config.bgCard} p-6 shadow-2xl transition-all`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-700/60">
        <div>
          <div className="text-xs sm:text-sm text-slate-400 uppercase tracking-wider font-bold">Pre-Submission Disposition</div>
          <div className="flex items-center gap-3 mt-1.5">
            <span className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-sm sm:text-base font-black tracking-wide border uppercase ${config.badgeClass}`}>
              {decisionKey === "RELEASE" && <ShieldCheck className="w-5 h-5" />}
              {decisionKey === "REVIEW" && <AlertTriangle className="w-5 h-5" />}
              {decisionKey === "HOLD_FOR_CORRECTION" && <ShieldAlert className="w-5 h-5" />}
              {decisionKey === "BLOCK_UNTIL_VALID" && <XCircle className="w-5 h-5" />}
              {config.label}
            </span>
            <span className="text-sm font-mono font-medium text-slate-300">Claim: {prediction.claim_id}</span>
          </div>
        </div>
        ...
        {onOpenWhatIf && decisionKey !== "RELEASE" && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black text-sky-100 bg-sky-500/20 hover:bg-sky-500/30 border border-sky-500/40 transition shadow-sm cursor-pointer"
          >
        ...
        <div className="bg-slate-900/90 p-5 rounded-xl border border-slate-800 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-sm sm:text-base font-bold text-slate-300">Predicted Denial Risk</span>
              <span className={`text-3xl sm:text-4xl lg:text-5xl font-black font-mono tracking-tight ${config.textClass}`}>
                {prediction.risk_score !== undefined ? `${(prediction.risk_score * 100).toFixed(1)}%` : "0.0%"}
              </span>
            </div>

            {/* Visual Meter Bar */}
            <div className="w-full h-3.5 bg-slate-800 rounded-full overflow-hidden mt-3 relative">
              {/* Threshold markers */}
          ...
          <div className="mt-3.5 pt-3 border-t border-slate-800/80 text-sm text-slate-300 leading-relaxed">
            <span className="text-slate-200 font-bold">Routing Policy: </span>
        ...
        <div className="bg-slate-900/90 p-5 rounded-xl border border-slate-800 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2.5">
              <span className="text-sm sm:text-base font-bold text-slate-300">Predicted Denial Code (CARC)</span>
            ...
            {prediction.predicted_reason_code ? (
              <div className="space-y-2">
                <div className="flex items-center gap-2.5">
                  <span className={`px-3 py-1 rounded-lg font-mono font-black text-base border ${carcMeta?.badgeColor || 'bg-slate-800 text-slate-200 border-slate-700'}`}>
                    {prediction.predicted_reason_code}
                  </span>
              ...
              <div className="py-3 flex items-center gap-2.5 text-emerald-400">
                <CheckCircle className="w-5 h-5 flex-shrink-0" />
                <span className="text-sm sm:text-base font-bold">No CARC denial risk identified. Clean claim structure.</span>
              </div>
            )}
          </div>

          <div className="mt-3.5 pt-3 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400 font-mono font-medium">
      ...
      {prediction.validation_warnings && prediction.validation_warnings.length > 0 && (
        <div
          ...
          className="mt-5 p-4 sm:p-5 bg-amber-500/15 border-2 border-amber-500/40 rounded-xl space-y-2.5 shadow-md"
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-amber-500/20 text-amber-200 border border-amber-500/40">
          ...
          <ul className="text-sm text-amber-200/95 list-disc list-inside space-y-1.5 pt-1">
