...
export default function EdiInspector({ claimData, onToast }) {
  ...
  return (
    <div className="space-y-4">
      {/* Top Toolbar (LEVEL 1: MAIN COMMAND) */}
      <div className="hud-card-major p-4 rounded-2xl flex flex-wrap items-center justify-between gap-3 text-xs animate-main-pulse">
        <div className="flex items-center space-x-3">
          <Terminal className="w-5 h-5 text-cyan-400 animate-pulse" />
          <div>
            <div className="flex items-center space-x-2">
              <span className="badge-major text-[9px]">
                ◈ Level 1: ANSI X12 EDI 837P Terminal
              </span>
              <span className="badge-sub text-[9px]">
                005010X222A1
              </span>
            </div>
            <span className="font-bold text-slate-100 uppercase tracking-wider block mt-0.5">
        ...
        <div className="flex items-center space-x-2">
          {ediData.has_validation_warning && (
            <span className="flex items-center space-x-1 px-3 py-1 rounded-full bg-rose-500/20 border border-rose-500/40 text-rose-300 font-bold text-[11px] animate-pulse">
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>Missing Segment Hazard</span>
            </span>
          )}

          <button
            onClick={handleCopy}
            className="hud-card-sub flex items-center space-x-1.5 px-3 py-2 rounded-xl text-slate-200 font-medium transition-colors"
          >
          ...
          <button
            ...
            className="btn-cyber flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-400 to-sky-500 text-slate-950 font-bold text-xs shadow-lg shadow-cyan-500/20 transition-all hover:scale-105 active:scale-95"
          >
