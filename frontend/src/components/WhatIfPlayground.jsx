  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-sky-500/40 rounded-2xl w-full max-w-4xl shadow-2xl p-6 relative my-8">
        ...
        <div className="flex items-center gap-3.5 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-xl font-black text-white flex items-center gap-2.5">
              ...
              <span className="px-2.5 py-1 rounded text-xs font-bold bg-sky-500/20 text-sky-300 border border-sky-500/30 font-mono">
        ...
        <div className="mt-5">
          <h3 className="text-sm font-black uppercase tracking-wider text-slate-300 mb-3.5 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold">Obtain Prior Auth</span>
                <input
                  type="checkbox"
                  checked={Boolean(modifiedClaim.prior_auth_flag)}
                  readOnly
                  className="rounded text-emerald-500 bg-slate-800 border-slate-600 pointer-events-none w-4 h-4"
                />
              </div>
              <p className="text-xs text-slate-300 mt-1.5">Simulates valid authorization number on file</p>
            </div>
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold">Re-run 270/271 Check</span>
                <input
                  type="checkbox"
              ...
              <p className="text-xs text-slate-300 mt-1.5">Re-verifies active member coverage today (0 days)</p>
            </div>
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold">Attach Clinical Notes</span>
                <input
                  type="checkbox"
              ...
              <p className="text-xs text-slate-300 mt-1.5">Attaches signed operative & clinical notes</p>
            </div>
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold">Attach Timely Proof</span>
                <input
                  type="checkbox"
              ...
              <p className="text-xs text-slate-300 mt-1.5">Provides proof of timely filing window compliance</p>
            </div>
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold">Append Modifier -76</span>
                <input
                  type="checkbox"
              ...
              <p className="text-xs text-slate-300 mt-1.5">Clarifies distinct repeat service vs duplicate</p>
            </div>
            ...
            <div className="flex items-center justify-center p-2">
              <button
                ...
                disabled={loading}
                className="w-full h-full min-h-[52px] flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-black text-sm shadow-lg shadow-sky-500/25 transition cursor-pointer"
              >
                {loading ? (
          ...
          <div className="mt-6 pt-5 border-t border-slate-800 animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-5 rounded-xl bg-slate-950/60 border border-slate-800">
                <div className="text-sm font-bold text-slate-400 uppercase">Original Pre-Submission State</div>
                <div className="flex items-baseline gap-2.5 mt-2">
                  <span className="text-4xl font-black font-mono text-rose-400">
                    {formatPercentage(simulationResult.original.risk_score)}
                  </span>
                  <span className="text-sm font-semibold text-slate-400">Denial Probability</span>
                </div>
                <div className="mt-2.5">
                  <span className={`px-3 py-1 rounded-lg text-xs sm:text-sm font-black border uppercase ${origRouting?.badgeClass}`}>
                    {simulationResult.original.routing_decision}
                  </span>
                </div>
                {simulationResult.original.predicted_reason_code && (
                  <div className="mt-2.5 text-sm font-mono text-slate-300">
              ...
              <div className="p-5 rounded-xl bg-emerald-950/20 border border-emerald-500/40 relative overflow-hidden">
                <div className="text-sm font-bold text-emerald-400 uppercase">Remediated State (After Proposed Fix)</div>
                <div className="flex items-baseline gap-2.5 mt-2">
                  <span className="text-4xl font-black font-mono text-emerald-400">
                    {formatPercentage(simulationResult.modified.risk_score)}
                  </span>
                  <span className="text-sm font-semibold text-emerald-300">Denial Probability</span>
                </div>
                <div className="mt-2.5">
                  <span className={`px-3 py-1 rounded-lg text-xs sm:text-sm font-black border uppercase ${modRouting?.badgeClass}`}>
                    {simulationResult.modified.routing_decision}
                  </span>
                </div>
                <div className="mt-2.5 text-sm text-emerald-200 font-medium leading-relaxed">
            ...
            {/* Impact Banner */}
            <div className="mt-5 p-5 rounded-xl bg-gradient-to-r from-emerald-950/40 to-sky-950/40 border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                <div className="p-2.5 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/40">
                  <ShieldCheck className="w-6 h-6" />
                </div>
                <div>
                  <div className="text-base font-black text-white flex items-center gap-2.5">
              ...
              <button
                ...
                className="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-sm shadow-lg shadow-emerald-500/25 transition cursor-pointer"
              >
